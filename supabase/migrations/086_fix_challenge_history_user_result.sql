-- Fix: Return the result from the user's perspective (win/loss/draw)
-- instead of raw game_result enum (white_wins/black_wins).
...
BEGIN
    ...
    SELECT
        ...
        CASE
            ...
            WHEN g.result = 'abandoned' THEN 'abandoned'
            WHEN g.result = 'white_wins' AND g.white_player_id = p_user_id THEN 'win'
            WHEN g.result = 'black_wins' AND g.black_player_id = p_user_id THEN 'win'
            ELSE 'loss'
        END AS game_result
    FROM challenges c
