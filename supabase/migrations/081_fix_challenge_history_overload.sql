-- Fix: Drop the 3-parameter overload of get_challenge_history that conflicts
-- with the 4-parameter version (which includes p_status).
-- The PGRST203 error occurs because PostgREST cannot disambiguate between:
--   get_challenge_history(p_user_id, p_limit, p_offset)
--   get_challenge_history(p_user_id, p_status, p_limit, p_offset)

-- Drop the 3-parameter variant if it exists
DROP FUNCTION IF EXISTS get_challenge_history(UUID, INTEGER, INTEGER);

-- Ensure the canonical 4-parameter version exists
